...
def get_fret(target, string):
    ...
    position = chromatic_scale[target]
    string_position = chromatic_scale[string]
    fret = (position - string_position) % 12
...
def parse_args():
    ...
    parser.add_argument("target", type=str, help="The note whose position you want to find")
    parser.add_argument("strings", nargs="+", type=str, help="List of open string notes")
    args = parser.parse_args()
    return args

if __name__ == "__main__":
    args = parse_args()
    targetnote = args.target
    open_trings = args.strings

    fret_positions = get_frets(target_ote, openstrings)

    print(f"{targetnote} is")
    for string, fret in fret_positions.items():
